...
plot_jail_pop_for_us <- function()  {
  jail_chart <- get_year_jail_pop %>%
    ...
    labs(x = "Year", y = "Total Jail Population", title = "Jail Population in US by Year", caption = "Jail population growth from 1970-2018") +
    geom_col() +
...
plot_jail_pop_by_states <- function(states) {
  jail_state_chart <- ggplot(get_jail_pop_by_states, 
    ...
    labs(x = "Year", y = "Total Jail Population", title = "Jail Population in States by Year", caption = "Jail pop. growth of WA, AL, and LA from 1970-2018") +
    geom_line()
...
popjail_plot <- function() {
  pop_plot <- ggplot(pop_vs_jail_prop, aes(x = year)) +
    ...
    labs(x = "Year", y = "Proportion of Black Population", title = "Black Pop Growth vs. Black Jail Growth", caption = "Prop. of black pop. growth (red line) vs. prop. of black incarceration growth (blue line)")
  return(pop_plot)
}
...
pop_vs_jail2 <- function() {
  popdf2 <- incarceration_trends %>%
    group_by(year) %>%
    summarize(sum(total_pop_15to64, na.rm = TRUE), sum(white_pop_15to64, na.rm = TRUE), sum(total_jail_pop, na.rm = TRUE), sum(white_jail_pop, na.rm = TRUE))
  colnames(popdf2)[2] = "pop_by_year2"
  colnames(popdf2)[3] = "whitepop_by_year"
  colnames(popdf2)[4] = "jail_by_year2"
  colnames(popdf2)[5] = "whitejail_by_year"
  return(popdf2)
}
pop_vs_jail2 <- pop_vs_jail2()
#View(pop_vs_jail2)

pop_vs_jail_prop2 <- function() {
  prop2 <- pop_vs_jail2 %>%
    group_by(year) %>%
    summarize(whitepop_by_year / pop_by_year2, whitejail_by_year / jail_by_year2)
  colnames(prop2)[2] = "pop_prop2"
  colnames(prop2)[3] = "jail_prop2"
  return(prop2)
}
pop_vs_jail_prop2 <- pop_vs_jail_prop2()
#View(pop_vs_jail_prop2)


popjail_plot2 <- function() {
  pop_plot2 <- ggplot(pop_vs_jail_prop2, aes(x = year)) +
    geom_line(aes(y = pop_prop2), color = "red") +
    geom_line(aes(y = jail_prop2), color = "blue") +
    labs(x = "Year", y = "Proportion of White Population", title = "White Pop Growth vs. White Jail Growth", caption = "Prop. of white pop. growth (red line) vs. prop. of white incarceration growth (blue line)")
  return(pop_plot2)
}
popjail_plot2 <- popjail_plot2()
#plot(popjail_plot2)
...
map_plot <- plot_usmap(data = black_jailandpop, values = "pop_minus_jail", color = "red") + 
  labs(title = "Black Population and Incarceration Difference by State", caption = "Geographical heatmap based on the diff. of black population and black incarceration") +
  scale_fill_continuous(
    low = "red", high = "white", name = "Black Pop.  vs. Black Incarcerated Pop.", label = scales::comma
